...
function xmldb_certificate_upgrade($oldversion=0) {
    ...
    if($oldversion < 2017011901) {
        $table = new xmldb_table('certificate');
        $field = new xmldb_field('cutoffdate', XMLDB_TYPE_INTEGER, '10', null, false, 0, 0);
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        upgrade_mod_savepoint(true, 2017011901, 'certificate');
    }
    if($oldversion < 2017011902) {
        $table = new xmldb_table('certificate');
        $field = new xmldb_field('mingrade', XMLDB_TYPE_INTEGER, '4', null, false, 0, 0);
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        upgrade_mod_savepoint(true, 2017011902, 'certificate');
    }

    if ($oldversion < 2017041801) {

        // Define field forumgrades to be added to certificate.
        $table = new xmldb_table('certificate');
        $field = new xmldb_field('forumgrades', XMLDB_TYPE_BINARY, null, null, null, null, null, 'mingrade');

        // Conditionally launch add field forumgrades.
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        // Define field forumpercent to be added to certificate.
        $table = new xmldb_table('certificate');
        $field = new xmldb_field('forumpercent', XMLDB_TYPE_INTEGER, '4', null, null, null, '10', 'forumgrades');

        // Conditionally launch add field forumpercent.
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        // Certificate savepoint reached.
        upgrade_mod_savepoint(true, 2017041801, 'certificate');
    }
