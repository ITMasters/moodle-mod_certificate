...
function certificate_print_attempts($course, $certificate, $attempts) {
    ...
    foreach ($attempts as $attempt) {
        ...
        $datecompleted = certificate_itm_attempt_date($certificate, $course);
        $row[] = $datecompleted;

        if ($gradecolumn) {
            $attemptgrade = certificate_get_itm_grade($certificate, $course);
            $row[] = $attemptgrade;
        }
...
function certificate_itm_mingrade_met($certificate, $course, $userid = null, $valueonly = false) {
    global $USER, $DB;
    ...
    if ($certificate->printgrade > 0) {
        ...
        $rs = $DB->get_recordset_sql('
                SELECT qa.sumgrades, qa.timefinish, qa.userid, gi.grademax
                FROM mdl_quiz_attempts qa
                LEFT JOIN mdl_grade_items gi ON gi.iteminstance = (SELECT instance FROM mdl_course_modules WHERE id = ?) AND gi.itemmodule = \'quiz\'
                WHERE quiz = (SELECT instance FROM mdl_course_modules WHERE id = ?) AND userid = ?', array($certificate->printgrade,$certificate->printgrade,$userid));
        foreach ($rs as $record) {
            if($record->timefinish < $certificate->cutoffdate && $record->sumgrades > $highestraw){
                $highestraw = $record->sumgrades;
                //$highestpercent = ($record->sumgrades / $record->grademax) * 100;
                $highestpercent = certificate_get_itm_grade($certificate,$course,null,true);
            }
        }
        $rs->close();
...
function certificate_itm_attempt_date($certificate, $course, $userid = null, $valueonly = false) {
    global $USER, $DB;
...
function certificate_get_itm_grade($certificate, $course, $userid = null, $valueonly = false) {
    ...
    if ($certificate->printgrade > 0) {
        ...
        if ($highestpercent == 0) {
            ...
            print_r($userid);
             $rs = $DB->get_recordset_sql('
                ...
                WHERE  itemid = (SELECT id FROM learnitm_mdln1.mdl_grade_items where itemmodule = \'course\' and courseid = ? and itemname = null) AND userid = ?', array($certificate->course,$userid));
              foreach ($rs as $record) {
        ...
        if($certificate->forumgrades != NULL && $certificate->forumgrades == true){
            ...
            $rs->close();
            $totalforums = count($courseforums);
            $userforumcount = 0;
            for ($i = 0; $i < $totalforums; $i++) {
                if(in_array($courseforums[$i],$userforums)){
                    $userforumcount++;
                }
            }
