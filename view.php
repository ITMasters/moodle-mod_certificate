...
if (empty($action)) { // Not displaying PDF
    ...
    echo $OUTPUT->heading(get_string('certheading', 'certificate'));
    if (!empty($certificate->intro)) {
        echo $OUTPUT->box(format_module_intro('certificate', $certificate, $cm->id), 'generalbox', 'intro');
    }
    echo '<h5>Last date that students can sit the exam and be issued a certificate: ' . userdate($certificate->cutoffdate) . '</h5>';
